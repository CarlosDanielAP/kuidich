...
namespace ChaserSangreLimpiaStates
{
    ...
    public class ChaseBall : State
    {
        ...
        public override void Reason(GameObject objeto)
        {
            ...
            else
            {
                ...
                if ((player.myTeam as TeamLosSangreLimpia).isTeammate(
    ...
    public class SearchGoal : State
    {
        ...
        public override void OnEnter(GameObject objeto)
        {
            player.steering.maxSpeed = 50;
            // Se supone que tengo la pelota, entonces decido ir tras 
            // el aro del rival
            int aro = Random.Range(0, 2);
            player.steering.Target = (player.myTeam as TeamLosSangreLimpia).rivalGoals[aro];
        ...
        public override void Reason(GameObject objeto)
        {
            ...
            if(Vector3.Distance(
                player.transform.position,
                player.steering.Target.position) < 20f) // calibrar
            {
                ...
                GameManager.instancia.Quaffle.GetComponent<Quaffle>().
                    Throw(
                        player.steering.Target.position - player.transform.position,
                               ((ChaserSangreLimpia)player).ThrowStrength
                    );
